...
-- this was an attempt at rightUpPush before I wrote push
function rightUpPush (p, iterations, scale)
	local scale = scale or 0.5
	if iterations <= 0 then
		return p
	else
		local newP = rightUpPush (p, iterations - 1, scale)
		local besideP = beside(p,newP, scale)
		return above(besideP,
					besideP,
					scale
					)  
	end
